...
async function selectStrategy(page: Page | ElementHandle, strategy: FingerprintStrategy) {
...
describe('visitorId', () => {
  async function runTest(page: Page, strategy: FingerprintStrategy, place: 'contentScript' | 'popup') {
    ...
    await selectStrategy(await page.waitForSelector('.fingerprint-container'), strategy)

    await getAndCheckResult(page)
  }

  describe('Iframe strategy', () => {
    extensionTest('should show visitorId in content script', async ({ page }) => {
